...
export default function NewResource() {
  ...
  useEffect(() => {
    fetchCategories(form.resource_type);
  }, []);
        ...
        <Section title="基本信息">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="资源标题" required>
              <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className="input" placeholder="输入资源标题" />
            </Field>
            ...
            <Field label="作者/开发商/艺人">
              <input type="text" value={form.author} onChange={e => setForm(f => ({ ...f, author: e.target.value }))} className="input" placeholder="如: 作者名/开发商/艺人" />
            </Field>
            <Field label="分类">
            ...
            <Field label="标签">
              <input type="text" value={form.tags} onChange={e => setForm(f => ({ ...f, tags: e.target.value }))} className="input" placeholder="逗号分隔，如: 动作,RPG,热门" />
            </Field>
            <Field label="封面图URL">
              <input type="text" value={form.cover_url} onChange={e => setForm(f => ({ ...f, cover_url: e.target.value }))} className="input" placeholder="图片链接地址" />
            </Field>
            <Field label="解锁积分">
              <input type="number" value={form.unlock_points} onChange={e => setForm(f => ({ ...f, unlock_points: parseInt(e.target.value) || 0 }))} className="input" placeholder="0=免费" />
            </Field>
          ...
          <Field label="描述" className="mt-4">
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} className="input min-h-[100px]" placeholder="资源描述..." />
          </Field>
        </Section>
            ...
            {form.download_links.map((link, i) => (
              <div key={i} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                ...
                <label className="flex items-center gap-1 text-sm text-gray-600 mt-1 shrink-0">
                  <input type="checkbox" checked={link.is_free} onChange={e => updateLink(i, 'is_free', e.target.checked)} className="rounded" />
                  免费
                </label>

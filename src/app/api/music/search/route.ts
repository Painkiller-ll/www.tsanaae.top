...
// 音乐搜索 API - 多平台搜索(网易云+酷狗)
export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);
    const keyword = searchParams.get('keyword');
    const platform = searchParams.get('platform') || 'all'; // all/netease/kugou
    ...
    if (platform === 'all' || platform === 'netease') {
      promises.push(searchNetease(keyword).then((r) => { results.push(...r); }));
    }
...
async function searchKugou(keyword: string) {
  ...
  try {
    const searchUrl = `http://mobilecdn.kugou.com/api/v3/search/song?keyword=${encodeURIComponent(keyword)}&page=1&pagesize=20`;
    const resp = await fetch(searchUrl);
    const data = await resp.json() as Record<string, unknown>;
    ...
    for (const song of info.slice(0, 15)) {
      const songName = (song.songname as string) || '';

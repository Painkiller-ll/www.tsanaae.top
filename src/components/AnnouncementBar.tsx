...
export default function AnnouncementBar() {
  ...
  return (
    <div className={`mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-4`}>
      <div className={`flex items-center gap-3 rounded-xl border ${style.border} ${style.bg} px-4 py-3 transition-all`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground mt-0.5 truncate">{current.content}</p>
        </div>
        {announcements.length > 1 && (
          <div className="flex items-center gap-1 shrink-0">
            <button
              onClick={() => setCurrentIndex(prev => prev === 0 ? announcements.length - 1 : prev - 1)}
              className="h-6 w-6 flex items-center justify-center rounded hover:bg-white/5 text-muted-foreground hover:text-foreground transition-colors"
            >
        ...
        <button
          ...
          className="h-6 w-6 flex items-center justify-center rounded hover:bg-white/5 text-muted-foreground hover:text-foreground transition-colors shrink-0"
        >
